 ...
 * @author user
 */
public class graficoSituacaoPFMna3parte
{
    ...
    public graficoSituacaoPFMna3parte()
    {
        Conexao = new Conexao();
        try
        {
    ...
    {
            Integer contador=1;
            Double PFM1_1P=0.0,PFM0_1P=0.0,PFM1_2P=0.0,PFM0_2P=0.0,PFM1_3P=0.0,PFM0_3P=0.0;
            Double PFM1_4P=0.0,PFM0_4P=0.0,PFM1_5P=0.0,PFM0_5P=0.0,PFM1_6P=0.0,PFM0_6P=0.0;
            try
            {
                String Software="";
                switch (this.getSoftware())
            {
                ...
                String Selecao = "";
                Selecao = " SELECT \"CB_PESOFRAGMETODO\" FROM \"COMB10\"("+Software+") ORDER BY \"CB_MEDIAPRECISAO\" DESC ";
                ...
                while (Resultado.next())
                {
                    ...
                    if (contador<=24)
                    {
                        ...
                        {
                            case 0:   PFM0_1P++;  break;
                            case 1:   PFM1_1P++;  break;
                        }
                    }
                    ...
                    if ((contador>=25) && (contador<=48))
                    {
                        switch (Resultado.getInt(1))
                        {
                            case 0:   PFM0_2P++;  break;
                            case 1:   PFM1_2P++;  break;
                        }
                    ...
                    if ((contador>=49) && (contador<=72))
                    {
                        switch (Resultado.getInt(1))
                        {
                            case 0:   PFM0_3P++;  break;
                            case 1:   PFM1_3P++;  break;
                        }
                    ...
                    if ((contador>=73) && (contador<=96))
                    {
                        switch (Resultado.getInt(1))
                        {
                            case 0:   PFM0_4P++;  break;
                            case 1:   PFM1_4P++;  break;
                        }
                    ...
                    if ((contador>=97) && (contador<=120))
                    {
                        switch (Resultado.getInt(1))
                        {
                            case 0:   PFM0_5P++;  break;
                            case 1:   PFM1_5P++;  break;
                        }
                    ...
                    if ((contador>=121) && (contador<=144))
                    {
                        switch (Resultado.getInt(1))
                        {
                            case 0:   PFM0_6P++;  break;
                            case 1:   PFM1_6P++;  break;
                        }
                    }
                contador++;
                }
    ...
    public void montaComandosR()
    {
      // Gerando os comandos do R dinamicamente - segunda parte
        ...
        String nomeArquivo = "c:/analise/cmdr/comandos13.r";
        try
        {
            this.fw = new FileWriter(nomeArquivo, false);
            this.fw.write("jpeg(\"c:/analise/resultado/graficoSituacaoPFMna6parte.jpg\",width = 400, height = 400, quality = 100)"+"\n");
            this.fw.write("library(ggplot2)"+"\n");
            this.fw.write("library(sqldf)"+"\n");
            this.fw.write("df = read.csv('c:/analise/bdados/graficoSituacaoPFMna6parte.csv',header=TRUE, sep=\";\")"+"\n");
	    ...
	    this.fw.write("WHEN Parte6==5 THEN '5P'"+"\n");
            ...
            this.fw.write("CASE WHEN PFM==0  THEN 'PFM0'" +"\n");
            this.fw.write("WHEN PFM==1  THEN 'PFM1' "+"\n");
            this.fw.write("END pfm from df\")"+"\n");
            this.fw.write("p = ggplot(data=df, aes(x=factor(1),y=Frequencia, fill = factor(pfm)),)" +"\n");
    ...
    public void executaComandosR()
    {
        ...
        try
        {
            Process p = Runtime.getRuntime().exec("R CMD BATCH c:\\analise\\cmdr\\comandos13.r");
            p.waitFor();
    ...
    public void visualizarGrafico()
    {
        ...
        try
        {
            Process p = Runtime.getRuntime().exec("xnview c:\\analise\\resultado\\graficoSituacaoPFMna6parte.jpg");
        } catch (Exception err)
        {
            err.printStackTrace();
        }
