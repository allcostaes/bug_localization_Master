...
public class  gPCXPrecisao
{
    ...
    public void montarComandosR()
    {
        ...
        try
        {
            ...
            this.fw.write("jpeg(\"c:/analise/resultado/grafico.jpg\",width = 1500, height = 11000, quality = 100)" + "\n");
        ...
        String Selecao = " SELECT DISTINCT \"CB_VALORK\",\"CB_PESOMETODO\",\"CB_PESOFRAGMETODO\",\"CB_PESOFRAGCLASSE\",\"CB_STOPWORDS\" FROM \"COMBINACOES\"";
        try
        {
        ...
        try
        {
            ...
            while (Resultado1.next())
            {
                ...
                try
                {
                    ...
                    this.fw.write("plot(0,0,type=\"b\",lty=1,main=heading,xlab=\"Peso Classe\",ylab=\"Precisão\",ylim=c(0.0,1.0),xlim=c(-0.2,1.5))" + "\n");
    ...
    private String obtemXY(Integer K, Integer PM, Integer PFM, Integer PFC, Integer STW)
    {
        String Selecao="";
        String XY="";
        Selecao = " SELECT \"TL_PESOCLASSE\", \"TL_MEDIAPRECISAO\", (SELECT MAX(\"PR\".\"PR_POSICAO\") FROM  \"PREC_REVOCACAO\" \"PR\" WHERE \"PR\".\"TL_CODIGO\"=\"TL\".\"TL_CODIGO\") AS \"MAX_POSICAO\"" +
        " FROM \"TESTELSI\"   \"TL\" WHERE  \"TL\".\"TL_VALORK\"=" + K + " AND  \"TL\".\"TL_PESOMETODO\"=" + PM +
        " AND  \"TL\".\"TL_PESOFRAGMETODO\"=" + PFM + " AND  \"TL\".\"TL_PESOFRAGCLASSE\"=" + PFC  + " AND  \"TL\".\"TL_STOPWORDS\"=" + STW;
        ...
        Selecao = Selecao + "  ORDER BY \"BG_CODIGO\", \"TL_PESOCLASSE\" ";
        String x = "", y = "";
        String Legenda="";
        ...
        try
        {
            ...
            String temporario = "";
            temporario = temporario+"PC=0;PC=1" + "\n";
            ...
            Unts = Unts + "legend(\"topright\", c(\"PC = 0 ["+unts_0+"]\",\" PC = 1  ["+unts_1+"]\"), cex=1.3, bty=\"n\", fill=rainbow(2))"+ "\n";
        } catch (IOException e)
        {
            e.printStackTrace();
        }
    ...
    private String obtemQuartis(Integer K, Integer PM, Integer PFM, Integer PFC, Integer STW)
    {
        String Selecao="";
        String Quartis="";
        Selecao = " SELECT \"TL_PESOCLASSE\", \"TL_MEDIAPRECISAO\" FROM \"TESTELSI\" WHERE \"TL_VALORK\"=" + K + " AND \"TL_PESOMETODO\"=" + PM +
        " AND \"TL_PESOFRAGMETODO\"=" + PFM + " AND \"TL_PESOFRAGCLASSE\"=" + PFC  + " AND \"TL_STOPWORDS\"=" + STW;
        if (this.getSoftware() == 1)
        {
           Selecao = Selecao + " AND \"BG_CODIGO\">=1  AND \"BG_CODIGO\"<=10";
        }
        ...
        try
        {
            ...
            Quartis = Quartis + "barplot(as.matrix(quartis), main=\"Distribuição de PC pelos Quartis\", xlab= \"Quartis\", ylab= \"Frequencia\", beside=TRUE, col=rainbow(2))"+ "\n";
            Quartis = Quartis + "legend(\"topright\", c(\"PC=0\",\"PC=1\"), cex=1.3, bty=\"n\", fill=rainbow(2))"+ "\n";
    ...
    private String obtemPorcentagemVariacao()
    {
            ...
            porcentagemVariacao = porcentagemVariacao + "pie(valores, main=\"Variação do Peso da Classe de 0 para 1\", col=colors, labels=labels, cex=1.3)"+"\n";
            ...
            porcentagemVariacao = porcentagemVariacao + "labels <- round(valores/sum(valores) * 100, 1)"+"\n";
            porcentagemVariacao = porcentagemVariacao + "labels <- paste(labels, \"%\", sep=\"\")"+"\n";
            porcentagemVariacao = porcentagemVariacao + "pie(valores, main=\"Porcentagem de combinações em que o menor Trabalho\n foi observado em cada valor de PC\", col=colors, labels=labels, cex=1.3)"+"\n";
            porcentagemVariacao = porcentagemVariacao + "legend(\"topleft\", c(\"PC=0\",\"PC=1 \"), cex=1.2, fill=colors)"+"\n";
