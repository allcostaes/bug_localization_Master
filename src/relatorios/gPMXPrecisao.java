...
public class  gPMXPrecisao
{
    ...
    public void montarComandosR()
    {
        ...
        String Selecao = " SELECT DISTINCT \"CB_VALORK\",\"CB_PESOFRAGMETODO\",\"CB_PESOCLASSE\",\"CB_PESOFRAGCLASSE\",\"CB_STOPWORDS\" FROM \"COMBINACOES\"";
        try
        {
        ...
        try
        {
            ...
            while (Resultado1.next())
            {
                ...
                try
                {
                    String titulo = "<K=" + Resultado1.getInt(1) + "><PFM=" + Resultado1.getInt(2)
                    + "><PC=" + Resultado1.getInt(3) + ">\n<PFC=" + Resultado1.getInt(4)
                    + "><STW=" + Resultado1.getInt(5) + ">";
                    this.fw.write("heading = paste(\"" + titulo + "\")\n");
                    this.fw.write("plot(0,0,type=\"b\",lty=1,main=heading,xlab=\"Peso Método\",ylab=\"Precisão\",ylim=c(0.0,1.0),xlim=c(-0.2,1.5))" + "\n");
    ...
    private String obtemXY(Integer K, Integer PFM, Integer PC, Integer PFC, Integer STW)
    {
        String Selecao="";
        String XY="";
        Selecao = " SELECT \"TL_PESOMETODO\", \"TL_MEDIAPRECISAO\", (SELECT MAX(\"PR\".\"PR_POSICAO\") FROM  \"PREC_REVOCACAO\" \"PR\" WHERE \"PR\".\"TL_CODIGO\"=\"TL\".\"TL_CODIGO\") AS \"MAX_POSICAO\"" +
        " FROM \"TESTELSI\"   \"TL\" WHERE  \"TL\".\"TL_VALORK\"=" + K + " AND  \"TL\".\"TL_PESOFRAGMETODO\"=" + PFM +
        ...
        Selecao = Selecao + "  ORDER BY \"BG_CODIGO\", \"TL_PESOMETODO\" ";
        String x = "", y = "";
        String Legenda="";
        ...
        try
        {
            ...
            String temporario = "";
            temporario = temporario+"PM=0;PM=1" + "\n";
            ...
            Unts = Unts + "legend(\"topright\", c(\"PM = 0 ["+unts_0+"]\",\" PM = 1  ["+unts_1+"]\"), cex=1.3, bty=\"n\", fill=rainbow(2))"+ "\n";
        } catch (IOException e)
        {
            e.printStackTrace();
        }
    ...
    private String obtemQuartis(Integer K, Integer PFM, Integer PC, Integer PFC, Integer STW)
    {
        String Selecao="";
        String Quartis="";
        Selecao = " SELECT \"TL_PESOMETODO\", \"TL_MEDIAPRECISAO\" FROM \"TESTELSI\" WHERE \"TL_VALORK\"=" + K + " AND \"TL_PESOFRAGMETODO\"=" + PFM +
        ...
        try
        {
            ...
            Quartis = Quartis + "barplot(as.matrix(quartis), main=\"Distribuição de PM pelos Quartis\", xlab= \"Quartis\", ylab= \"Frequencia\", beside=TRUE, col=rainbow(2))"+ "\n";
            Quartis = Quartis + "legend(\"topright\", c(\"PM0\",\"PM1\"), cex=1.3, bty=\"n\", fill=rainbow(2))"+ "\n";
    ...
    private String obtemPorcentagemVariacao()
    {
            ...
            porcentagemVariacao = porcentagemVariacao + "pie(valores, main=\"Variação do Peso do Método de 0 para 1\", col=colors, labels=labels, cex=1.3)"+"\n";
            ...
            porcentagemVariacao = porcentagemVariacao + "labels <- round(valores/sum(valores) * 100, 1)"+"\n";
            porcentagemVariacao = porcentagemVariacao + "labels <- paste(labels, \"%\", sep=\"\")"+"\n";
            porcentagemVariacao = porcentagemVariacao + "pie(valores, main=\"Porcentagem de combinações em que a utilização\n de um PM resultou em menos trabalho\", col=colors, labels=labels, cex=1.3)"+"\n";
            porcentagemVariacao = porcentagemVariacao + "legend(\"topleft\", c(\"PM=0\",\"PM=1 \"), cex=1.2, fill=colors)"+"\n";
