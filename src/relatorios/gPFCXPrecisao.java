...
public class  gPFCXPrecisao
{
    ...
    private String obtemXY(Integer K, Integer PM, Integer PFM,Integer PC, Integer STW)
    {
        String Selecao="";
        String XY="";
        Selecao = " SELECT \"TL_PESOFRAGCLASSE\", \"TL_MEDIAPRECISAO\", (SELECT MAX(\"PR\".\"PR_POSICAO\") FROM  \"PREC_REVOCACAO\" \"PR\" WHERE \"PR\".\"TL_CODIGO\"=\"TL\".\"TL_CODIGO\") AS \"MAX_POSICAO\"" +
        " FROM \"TESTELSI\"   \"TL\" WHERE  \"TL\".\"TL_VALORK\"=" + K + " AND  \"TL\".\"TL_PESOMETODO\"=" + PM + " AND  \"TL\".\"TL_PESOFRAGMETODO\"=" + PFM +
        " AND  \"TL\".\"TL_PESOCLASSE\"=" + PC + " AND  \"TL\".\"TL_STOPWORDS\"=" + STW;
        ...
        Selecao = Selecao + "  ORDER BY \"BG_CODIGO\", \"TL_PESOFRAGCLASSE\" ";
        String x = "", y = "";
        String Legenda="";
        ...
        try
        {
            ...
            String temporario = "";
            temporario = temporario+"PFC=0;PFC=1" + "\n";
            ...
            Unts = Unts + "legend(\"topright\", c(\"PFC = 0 ["+unts_0+"]\",\" PFC = 1  ["+unts_1+"]\"), cex=1.3, bty=\"n\", fill=rainbow(2))"+ "\n";
        } catch (IOException e)
        {
            e.printStackTrace();
        }
    ...
    private String obtemQuartis(Integer K, Integer PM, Integer PFM, Integer PC, Integer STW)
    {
        String Selecao="";
        String Quartis="";
        Selecao = " SELECT \"TL_PESOFRAGCLASSE\", \"TL_MEDIAPRECISAO\" FROM \"TESTELSI\" WHERE \"TL_VALORK\"=" + K + " AND \"TL_PESOMETODO\"=" + PM +  " AND \"TL_PESOFRAGMETODO\"=" + PFM +
        " AND \"TL_PESOCLASSE\"=" + PC + " AND \"TL_STOPWORDS\"=" + STW;
        if (this.getSoftware() == 1)
        {
           Selecao = Selecao + " AND \"BG_CODIGO\">=1  AND \"BG_CODIGO\"<=10";
        }
    ...
    private String obtemPorcentagemVariacao()
    {
            ...
            porcentagemVariacao = porcentagemVariacao + "labels <- round(valores/sum(valores) * 100, 1)"+"\n";
            porcentagemVariacao = porcentagemVariacao + "labels <- paste(labels, \"%\", sep=\"\")"+"\n";
            porcentagemVariacao = porcentagemVariacao + "pie(valores, main=\"Porcentagem de combinações em que a utilização\n de um PFC resultou em menos trabalho\", col=colors, labels=labels, cex=1.3)"+"\n";
            porcentagemVariacao = porcentagemVariacao + "legend(\"topleft\", c(\"PFC=0\",\"PFC=1 \"), cex=1.2, fill=colors)"+"\n";
